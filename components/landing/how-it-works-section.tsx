          ...
          {steps.map((item, index) => (
            <div key={index} className="flex flex-col items-center text-center group">
              <div className="relative mb-5">
                <div className="w-16 h-16 rounded-2xl bg-white shadow-sm border border-gray-200/60 flex items-center justify-center transition-[background-color,box-shadow,transform,border-color] duration-300 group-hover:bg-[var(--bylink-primary)] group-hover:shadow-lg group-hover:shadow-[var(--bylink-primary)]/20 group-hover:scale-105 group-hover:border-transparent">
                  <item.icon className="h-7 w-7 text-[var(--bylink-primary)] transition-colors duration-300 group-hover:text-white" />
                </div>
                <div
                  aria-hidden="true"
                  className="absolute -top-2 -right-2 w-7 h-7 rounded-full bg-[var(--bylink-primary)] text-white font-bold text-sm flex items-center justify-center shadow-md transition-transform duration-300 group-hover:scale-110"
                >
                  {item.step}
                </div>
              </div>
