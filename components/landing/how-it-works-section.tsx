...
export function HowItWorksSection() {
  ...
  return (
    <section id="how-it-works" className="relative py-16 sm:py-24 px-4 sm:px-6 lg:px-8 bg-white overflow-hidden">
      {/* Ambient glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[300px] bg-[var(--bylink-primary)]/[0.03] rounded-full blur-3xl pointer-events-none" />
          ...
          {steps.map((item, index) => (
            <div key={index} className="flex flex-col items-center text-center group">
              <div className="relative mb-5">
                <div className="w-16 h-16 rounded-2xl bg-white shadow-sm border border-gray-200/60 flex items-center justify-center transition-all duration-300 group-hover:bg-[var(--bylink-primary)] group-hover:shadow-lg group-hover:shadow-[var(--bylink-primary)]/20 group-hover:scale-105 group-hover:border-transparent">
                  <item.icon className="h-7 w-7 text-[var(--bylink-primary)] transition-colors duration-300 group-hover:text-white" />
                </div>
                <div className="absolute -top-2 -right-2 w-7 h-7 rounded-full bg-[var(--bylink-primary)] text-white font-bold text-sm flex items-center justify-center shadow-md transition-transform duration-300 group-hover:scale-110">
                  {item.step}
                </div>
              </div>
