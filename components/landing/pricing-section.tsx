...
export function PricingSection() {
  ...
  return (
    <section id="pricing" ref={sectionRef} className="reveal relative py-16 sm:py-24 px-4 sm:px-6 lg:px-8 bg-gray-900 overflow-hidden">
      ...
      <div className="absolute top-1/2 right-1/3 -translate-y-1/2 w-[400px] h-[400px] bg-[#7C3AED]/[0.07] rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-[#7C3AED]/[0.05] rounded-full blur-3xl pointer-events-none" />

      <div className="max-w-4xl mx-auto relative">
        <div className="text-center mb-10 sm:mb-14">
          <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white text-balance">
            Empieza gratis.{" "}
            <span className="text-[#7C3AED]">Crece cuando quieras.</span>
          </h2>
          ...
          {plans.map((plan, index) => (
            <div
              ...
              className={`relative rounded-2xl p-6 sm:p-8 flex flex-col transition-all duration-300 ${
                plan.highlighted
                  ? "bg-gradient-to-br from-white via-white to-[#F5F0FF] ring-2 ring-[#7C3AED] scale-[1.02] sm:scale-105 shadow-[0_0_40px_rgba(124,58,237,0.15),0_0_80px_rgba(124,58,237,0.05)]"
                  : "bg-gray-800/80 border border-gray-700/50 opacity-90 hover:opacity-100 hover:border-gray-600"
              }`}
            >
              {plan.highlighted && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-[#7C3AED] to-[#6D28D9] text-white text-xs font-bold px-4 py-1.5 rounded-full shadow-lg shadow-[#7C3AED]/30 tracking-wide badge-glow">
                  Recomendado
                </div>
              )}
              ...
              <div className="mb-6">
                ...
                <div className="flex items-baseline gap-1">
                  {plan.highlighted ? (
                    <span className="text-4xl font-extrabold bg-gradient-to-r from-[#7C3AED] to-[#6D28D9] bg-clip-text text-transparent">
                      {plan.price}
                    </span>
                  ) : (
                  ...
                  <li key={i} className="flex items-start gap-2.5 text-sm group">
                    <Check
                      className={`h-4 w-4 mt-0.5 shrink-0 transition-transform duration-200 group-hover:scale-125 ${
                        plan.highlighted ? "text-[#7C3AED]" : "text-gray-400"
                      }`}
                    />
              ...
              {plan.isRegister ? (
                ...
                <Link href={isAuthenticated ? "/dashboard/plan" : "/registro"}>
                  <Button
                    className="w-full gap-2 cursor-pointer bg-[#7C3AED] hover:bg-[#6D28D9] text-white shadow-lg shadow-[#7C3AED]/25 btn-press btn-shimmer"
                    size="lg"
                  >
                    {plan.cta}
                    <ArrowRight className="h-4 w-4" />
