...
import { Check, MessageCircle, Mail, Send, Banknote, X, ArrowRight } from "lucide-react"
...
export function PricingSection() {
  const [modal, setModal] = useState<{ open: boolean; planName: string }>({ open: false, planName: '' })
  const [method, setMethod] = useState<ContactMethod>('whatsapp')
  const [contactValue, setContactValue] = useState('')
  const [description, setDescription] = useState('')
  const [transferInfo, setTransferInfo] = useState({ name: '', bank: '', date: '' })
  ...
  const handleSend = () => {
    if (method === 'whatsapp') {
      ...
      if (!contactValue.trim()) return
      const subject = encodeURIComponent(`Plan ${modal.planName} — Bio Link Store`)
        ...
        <div className="mt-8 max-w-5xl mx-auto relative overflow-hidden rounded-2xl">
          ...
          <div className="relative p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center gap-5 sm:gap-8">
            <div className="flex-1 text-center sm:text-left">
              <p className="text-xs font-semibold tracking-wider uppercase text-[#6ee490] mb-1">Plan personalizado</p>
              ...
              <p className="text-sm text-white/60 leading-relaxed max-w-lg mx-auto sm:mx-0">
                Elige solo las funciones que necesitas sin pagar de más. Diseño exclusivo, integraciones, reportes avanzados — armamos el plan perfecto para tu negocio.
              </p>
            </div>
            <Button
              size="lg"
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold tracking-wider uppercase text-[#6ee490]">
                  {modal.planName === 'Personalizado' ? 'Plan personalizado' : `Plan ${modal.planName}`}
                </p>
                <h3 className="text-lg font-bold text-white">
                  {modal.planName === 'Personalizado' ? 'Contanos qué necesitas' : '¿Cómo quieres continuar?'}
                </h3>
              </div>
            ...
            {method === 'transfer' ? (
              <>
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) => setTransferInfo(p => ({ ...p, name: e.target.value }))}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs text-white/40 font-medium">Banco</label>
                    <input
                      type="text"
                      value={transferInfo.bank}
                      onChange={(e) => setTransferInfo(p => ({ ...p, bank: e.target.value }))}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs text-white/40 font-medium">
                      Fecha de transferencia <span className="text-white/20">(opcional)</span>
                    </label>
                    <input
                      type="date"
                      value={transferInfo.date}
                      onChange={(e) => setTransferInfo(p => ({ ...p, date: e.target.value }))}
                ...
                {modal.planName === 'Personalizado' && (
                  <div className="space-y-1.5">
                    <label className="text-xs text-white/40 font-medium">
                      ¿Qué funciones necesitas? <span className="text-white/20">(opcional)</span>
                    </label>
                    <textarea
                      value={description}
              ...
              <>
                <div className="space-y-1.5">
                  <label className="text-xs text-white/40 font-medium">
                    {method === 'whatsapp' ? 'Tu número de WhatsApp' : 'Tu email'}
                  </label>
                  <input
                    type={method === 'whatsapp' ? 'tel' : 'email'}
                    value={contactValue}
                    onChange={(e) => setContactValue(e.target.value)}
                    placeholder={method === 'whatsapp' ? '+54 9 11 XXXX XXXX' : 'tu@email.com'}
                    className={`w-full px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-white/20 focus:outline-none focus:bg-white/8 transition-all ${
                      method === 'whatsapp' ? 'focus:border-[#25D366]/50' : 'focus:border-[#327be2]/50'
                    }`}
                  />
