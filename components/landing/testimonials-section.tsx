...
export function TestimonialsSection() {
  ...
  return (
    <section
      id="testimonials"
      className="py-12 sm:py-20 px-4 sm:px-6 lg:px-8 glass-panel"
    >
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10 sm:mb-16">
          <p className="text-sm text-[#33b380] font-semibold mb-2">
            Testimonios
          </p>
          ...
          <p className="text-white/80 max-w-2xl mx-auto">
            Más de 5,000 tiendas de Instagram confían en InstaShop para vender
            más.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className="p-6 rounded-2xl bg-[#0a0f14] border border-white/10"
            >
              <div className="flex items-center gap-1 mb-4">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star
                    key={i}
                    className="h-4 w-4 fill-[#6ee490] text-[#6ee490]"
                  />
                ))}
              </div>
