...
## B7 — Storefront v2 canónico (2026-07-09/10)

Bloque grande: llevar `/v2/[slug]` (el renderer con theming BE-120 + carrito real, que convivía como ruta paralela a la tienda legacy de 3 templates) a ser la tienda pública real y corregir todo lo que quedaba a medias en el camino.

**Tracking dual estaba roto en el sistema nuevo, no solo desincronizado del legacy:** `lib/storefront-tracking.ts` pegaba a `${apiUrl}/public/:slug/event` — sin el prefijo `/api` que **todo** el resto del backend usa. Todo evento del sistema de tracking nuevo (`sendBeacon` y el `fetch` de respaldo) fallaba en silencio contra un 404; nunca se notó porque el sistema legacy (`lib/analytics.ts`) sí funcionaba y enmascaraba el problema. Corregido en los dos puntos de la función.

**Bug de nombres de paleta, en tres capas a la vez:** `PaletteTokens` (tipo frontend) esperaba `background`/`textMuted`, pero el DTO real del backend (confirmado contra `store-preview-response.dto.ts`/seed) usa `bg`/`muted`/`secondary`. `tokens.ts` (`resolveTokens`) y `tokens-editor.tsx` (`PALETTE_KEYS`/swatches de presets) leían las claves viejas → cualquier paleta con esos campos silenciosamente caía a los defaults hardcodeados, y el editor de temas no mostraba ni permitía tocar `secondary`. Corregido el tipo y ambos consumidores contra el nombre real del backend, no al revés.

**`TemplateRenderer` no sabía renderizar la mayoría de lo que el editor de secciones ya permitía guardar:** hero (`headline`/`subheadline`), grid de productos (`layout` como enum real en vez de asumir un único grid, filtro por categoría, mostrar/ocultar precio y SKU), productos destacados (`productIds` como `{id}[]`, no ids sueltos), galería (`image`/`caption`), bloque de texto (`kicker`/`headline`), CTA banner (`headline`/`subline`/color de fondo), FAQ (`question`/`answer`) y mapa (lat/lng/zoom/título, con fallback a `store.address`). Sin esto, cualquier tienda que usara esas secciones veía huecos vacíos o texto genérico aunque el editor mostrara los datos correctos.

**`ProductCard` era un link muerto:** `<a href="#product-...">` sin ningún elemento con ese id — click no hacía nada. Reemplazado por `<button onClick={onOpen}>` y agregados los dos componentes que le faltaban al storefront nuevo para tener un flujo de compra real:
- `product-detail-sheet.tsx`: ficha de producto con selección de variante (match por combinación exacta contra `product.variants`), cantidad, agregar al carrito.
- `cart-sheet.tsx`: carrito con edición de cantidad y checkout que llama a `paymentProvider.checkout()` (WhatsApp) — antes no existía ningún carrito en el sistema nuevo, solo en el legacy.
- `storefront-client.tsx`: envuelve todo en `CartProvider` y conecta ambos sheets + el contador del carrito en la navbar.
- `lib/payment-providers/whatsapp.ts`: locale del mensaje generado estaba en `es-AR`, cambiado a `es-VE` (mercado real del producto).
- `lib/cart-context.tsx`: `addItem` ahora dispara ambos sistemas de tracking (legacy + nuevo), mismo patrón dual que el resto del storefront.

**Multi-moneda real en el storefront público:** nuevo fetch server-side a `/api/public/rates` (tasa BCV, sin auth) pasado como prop `rate`; toggle USD/Bs en la navbar solo aparece si `store.currency !== "VES"` y hay tasa disponible. Antes no existía ninguna forma de que un comprador viera precios en bolívares.

**Responsive real, no solo grids que se acomodan solas:** las secciones simétricas (grid de productos, galería, FAQ, categorías) ya usaban `repeat(auto-fit, minmax(Npx, 1fr))` y no necesitaban nada más. Hero y about son pares asimétricos texto+imagen que necesitan **apilarse**, no solo achicar columnas — se agregó un `@media (max-width: 760px)` real (inyectado vía `<style>`, el mismo patrón que ya usa `panel-official.tsx` desde B1) para esos dos casos + padding de sección + navbar.

**Preview de borrador real (no el switch de templates legacy):** el botón "Abrir preview" del editor de temas (`theme-editor.tsx`) abría `/v2/${slug}?preview=true`, pero esa página nunca leyó el query param — mostraba siempre el tema publicado. El endpoint que sirve el borrador (`GET /stores/:storeId/theme/preview`) es autenticado (`JwtAuthGuard`+`StoreOwnerGuard`, del controller del dashboard, no del público) por diseño — no se tocó el guard ni se creó ningún endpoint público nuevo, eso sería trabajo de seguridad y quedó fuera de alcance esta sesión. En su lugar, `lib/page-builder-api/draft-preview.ts` (server-only, no exportado desde el barrel `index.ts` para no romper el bundle de cliente — usa `next/headers`) lee la misma cookie httpOnly (`igs_at`) que ya usa `/api/proxy` para el dashboard, y llama al endpoint autenticado directo desde el server component de la tienda. Si no hay cookie o el backend la rechaza (sesión expirada, no es el dueño), cae al tema publicado sin ningún cambio de comportamiento para el público — cero superficie nueva sin auth.

**Swap de ruta canónica:** `/[slug]` (antes: `StorePageClient` + selector legacy de 3 templates vía `?preview=templateId`) ahora es la misma página que vivía en `/v2/[slug]` — theming BE-120, carrito, preview de borrador, tasa BCV, todo. El fallback a `StorefrontDemo` se mantiene solo por robustez defensiva (el backend siempre devuelve un tema, así que en la práctica es inalcanzable, igual que antes). `/v2/[slug]` pasa a ser un redirect a `/[slug]` preservando query string, por si quedan bookmarks o links viejos. El único link interno que apuntaba a `/v2/...` (`theme-editor.tsx`) se actualizó a la ruta canónica. `StorePageClient` y el switch `?preview=templateId` quedan sin ningún caller — no se borraron en este bloque (van en B8, para no mezclar el swap funcional con la limpieza de código muerto).

**Metadata/OG:** la página vieja en `/[slug]` ya tenía OG + Twitter card completos (con la marca corregida en B2); la de `/v2/[slug]` no tenía nada. Se conservó la versión rica (con `alternates.canonical`) como la definitiva.

**Tipografía de tema nunca se cargaba:** los 9 templates declaran fuentes editoriales (Playfair Display, Fraunces, Lora, etc.) vía `ThemeTokens.typography`, pero nada las cargaba — ni `next/font` ni un `<link>` a Google Fonts. Cada tienda con una fuente no-Inter caía silenciosamente al fallback `sans-serif`/`monospace` del navegador; visualmente todos los templates se veían con la misma tipografía sin importar lo que el editor mostrara. `next/font/google` no aplica aquí porque la fuente se resuelve en runtime por tienda, no es estática en build. Fix: `lib/google-fonts.ts` construye la URL de Google Fonts CSS2 para las 3 fuentes resueltas (heading/body/mono), cargada vía `<link rel="stylesheet" precedence="default">` en la raíz de `TemplateRenderer` (React 19 la deduplica/hoistea al `<head>`).

**Verificación:** frontend `npx tsc --noEmit` y `pnpm build` limpios (mismas 39 rutas que B6 — `/v2/[slug]` pasa de página completa a redirect pero sigue siendo una entrada de ruta).

---

## B8 — Limpieza de código muerto (2026-07-10)

Cada eliminación se verificó primero con grep de importadores reales (no solo el nombre del archivo) antes de borrar, y `npx tsc --noEmit` + `pnpm build` corrieron limpios al final. El alcance terminó siendo mayor al de la lista original del plan — al trazar cada cluster aparecieron más árboles huérfanos de los previstos.

**Cluster v0 (`components/cart-sheet.tsx`, `cart-button.tsx`, `cart-context.tsx`, `product-card.tsx`, `product-grid.tsx`, `store-header.tsx`, `search-bar.tsx`, `category-filter.tsx`, `lib/store-data.ts`):** confirmado que solo se importaban entre sí, cero importadores externos. Eliminado el cluster completo.

**Templates legacy (`components/templates/**`, 34 archivos: vitrina/luxora/noir/menu/servicios/inmuebles + `renderer.tsx`, `template-gallery.tsx`, `template-card.tsx`, `template-data.ts`, `custom-cta-versions.tsx`, `custom-template-card.tsx`, `template-preview-modal.tsx`, `product-detail-renderer.tsx`, `shared/*`):** huérfanos tras el swap de ruta canónica de B7 (su único consumidor real, `components/store-page-client.tsx`, ya no tiene caller). También eliminado `store-page-client.tsx` mismo. Único cabo suelto: `components/product-detail-client.tsx` (usado por `/[slug]/[productSlug]`, la página de producto individual del sistema viejo) importaba `product-detail-renderer.tsx` de este árbol — ver abajo.

**Página de producto individual (`/[slug]/[productSlug]`) — decisión explícita, no solo borrado:** confirmé que **nada** en el storefront v2 genera links a esta ruta (`ProductCard` desde B7 abre un sheet in-page, no navega) — ni siquiera antes de esta sesión: solo los `product-card.tsx` de los templates legacy (ya muertos) construían ese href. No es una regresión de hoy, ya era funcionalidad muerta. En vez de dejarla rota (con el look del template legacy, inconsistente visualmente con el resto), la convertí en `redirect(`/${slug}`)`. **Nota para Antonio:** si se quiere un link compartible por producto individual (relevante para un negocio de WhatsApp-commerce), es una feature nueva a diseñar — el sheet actual no tiene URL propia. Eliminados también `product-detail-client.tsx`, `getProductBySlug`/`adaptProductDetail` de `lib/api.ts`, y los tipos `ProductDetail`/`ProductAttribute`/`ProductVariantDetail`/`VariantDetail`/`ProductVariant` (legacy) de `lib/types.ts` — ninguno tenía otro consumidor.

**Auth mobile — hallazgo no listado en el plan original:** al rastrear por qué `TemplateId`/`store.template` seguía leyéndose en algunos lados, encontré `components/auth-v2/auth-mobile/` (587 líneas, flow de registro+onboarding completo, montado solo en `/acceso/movil`) sin ningún caller real — nada redirige ahí por viewport/dispositivo. Confirmé con `git log` que no es un flujo mobile activo sin terminar: el commit `6ab8fd2` ("use auth-desktop flow as canonical login + registro") dice explícitamente *"Old LoginForm/RegisterForm left untouched (Fase 2 rewire)"* — es decir, ya estaba marcado para esta sesión. `auth-desktop` sí es responsive de verdad (`@media (max-width: 900px)` real en `data.ts`, colapsa a una columna) — no hay gap de mobile, solo el componente viejo sin usar. Eliminados `components/auth-v2/auth-mobile/`, `/acceso/movil`, y por la misma cadena `components/auth/register-form.tsx` (único caller: el método `register()` de `auth-context.tsx`, que solo servía a este flujo — eliminado junto con su push a `/onboarding`) y `components/auth/login-form.tsx` (cero callers). `app/onboarding/page.tsx` (con `TemplateGallery`/`CustomDesignBar`) solo era alcanzable desde ese `register()` muerto — eliminado también.

**Editores legacy de diseño:** `app/dashboard/diseno` (258 líneas, editor viejo de nombre/slug/colores/logo con `EditorPanel`/`PreviewPanel` y productos demo) y `app/dashboard/plantillas` (galería de 9 templates vieja) quedaron huérfanos tras B5/B7 — el diseño real vive en la vista `design` del panel v2 (`DesignBoard` → `ThemeEditor`, con su propio `TemplatesSidebar`). Convertidos a `redirect("/dashboard?view=design")`, mismo patrón que `/dashboard/productos` etc. desde B6. Eliminado `components/dashboard/design-editor/*` (editor-panel, preview-panel, demo-products, mock-providers) — sin otro caller.

**Landing pre-v2 — otro árbol no listado en el plan:** al revisar quién usa `components/ui/button.tsx` para decidir la consolidación de botones, encontré que `components/landing/{navbar,hero-section,pricing-section,cta-section,features-section,how-it-works-section,testimonials-section,stats-section,footer}.tsx` (9 archivos) tienen cero importadores — `landing-v2/landing-page.tsx` (la que realmente renderiza `/`) tiene su propio markup inline, no los usa. Solo `components/landing/landing-tracker.tsx` es real (usado por `app/page.tsx`/`app/home/page.tsx`). Eliminados los 9 archivos muertos.

**`orders.http-repository.ts` — el hack de `tokenStorage` era inofensivo pero incorrecto:** `(this.http as any).tokenStorage?.getAccessToken?.()` apuntaba a una propiedad que no existe en `HttpClient` desde la reescritura a cookies httpOnly + `/api/proxy` — siempre devolvía `undefined`. No rompía `exportCsv()` en la práctica porque el proxy inyecta su propio `Authorization` desde la cookie y **ignora** cualquier header que mande el cliente — pero el código mentía sobre cómo se autentica. Quitado el header manual (innecesario) y expuesto `HttpClient.baseUrl` como `readonly` público (antes `private`, forzando el `as any` para `exportCsv`'s fetch crudo).

**`app/page.tsx` vs `app/home/page.tsx`:** contenido idéntico (misma `LandingPage`+`LandingTracker`), dos URLs para la misma landing → contenido duplicado para SEO. `/` es la raíz natural y ya es el único destino que usa `signup-desktop.tsx` (el flow demo de `/signup-demo`); eliminado `app/home/page.tsx`. `/home/terminos` (ruta anidada real, linkeada desde 3 lugares live) no se tocó — sigue funcionando igual sin necesidad de un `page.tsx` en `/home` mismo. El único link a `/home` (bare) que existía, en `forgot-password-form.tsx` ("Volver al inicio"), se corrigió a `/`.

**Diferido — consolidación de botones:** el plan pedía unificar los "3 sistemas de botón" (clases `.btn`/`.ad-btn`, `components/ui/button.tsx` shadcn, `components/bylink/Button`) en el primitivo v2. Tras limpiar los clusters muertos arriba, `ui/button.tsx` quedó con solo 6 callers reales, todos en páginas explícitamente fuera del alcance de fase2 (no migradas a v2 en ningún bloque anterior): `forgot-password-form.tsx`, `reset-password-form.tsx`, `dashboard/store-switcher.tsx` (sidebar legacy), `dashboard/rates-config-card.tsx`, `ui/alert-dialog.tsx`, y las páginas grandes `/dashboard/plan` (748 líneas) y `/dashboard/pagos` (745 líneas) — ninguna tocada en esta sesión. Migrar sus botones es un refactor real (traducir props `variant`/`size`/`asChild` de shadcn al API de `bylink/Button`) sobre código que hoy funciona y no se verificó visualmente en este bloque — el riesgo de regresión no se justifica solo por consistencia interna. Queda pendiente para cuando esas páginas mismas se migren a v2 (no hay ticket de eso en este plan).

**Verificación con grep guards del plan (§5.2):** corridos todos — `biolinkstore`, `Bio Link Store`, `#33b380`, `es-AR`, `ARS`, `MOCK_`, `847`, `48.32`, `María Alvarado`, `demo-toggle`, `Saltar a`. Casi todos en 0 o en contenido de demo intencional (`MOCK_SUMMARY`/`847` solo alcanzable desde `/analytics-demo` sin prop `data`; `RATE = 48.32`/mock store solo desde `/tienda-demo` sin prop `data`; `Saltar a` correctamente gateado por `showScreenJumper` desde B1). Tres hits reales, no cosméticos:
- `app/onboarding/layout.tsx` seguía con `"Configura tu tienda - biolinkstore"` — pero al revisar quién lo necesita, sus dos únicos hijos ya no existen (`page.tsx` bare, borrado arriba) o son un redirect puro (`create-store`), así que ese layout nunca llega a pintarse para un usuario real (Next descarta el árbol al hacer `redirect()`). Eliminado junto con `components/brand/logo.tsx` (su único importador) y, al revisar ese hallazgo, `components/dashboard/rates-config-card.tsx` (cero importadores, huérfano de algún bloque anterior).
- `lib/payment-providers/whatsapp.ts`: el default del constructor era `currency = 'ARS'` (pesos argentinos, para un producto 100% venezolano) — ninguno de los 2 call sites reales lo deja caer en el default (ambos pasan `store.currency` explícito), pero es un default objetivamente incorrecto si algo lo llama sin argumento en el futuro. Cambiado a `'USD'`, consistente con el resto del código.

**Verificación:** frontend `npx tsc --noEmit` limpio · `pnpm build` limpio, 36 rutas (`layout.tsx` no cuenta como ruta propia, así que el conteo no cambia respecto al primer pase de B8 — solo bajó código muerto).

---

## Iteración 3 — Galería de temas completa + framework (2026-07-16)

**Fase 0 — Push de seguridad.** Se subieron TODAS las ramas locales de ambos repos a origin (13 frontend + 48 backend) — incluidas `feat/multi-currency-rates` y `rebrand/bylink-domain-swap`, que eran las únicas copias de los temas curados `estate` y `persona` (React hand-crafted nunca pusheado). Riesgo de pérdida total eliminado.

**Fase 1 — Port de estate y persona (galería con 11 temas).** Inventario previo: 9 temas ya estaban portados al sistema de secciones (BE-120); faltaban estate (inmuebles premium navy/dorado) y persona (portafolio estilo perfil social). Ambos portados como TemplateSeed siguiendo el molde atelier/rosier, con niche nuevo `PORTFOLIO` end-to-end (enum + migración `20260716000245`, union del frontend, chip en la galería — los chips salen de una constante hardcodeada, no de los datos) y `demoPortfolio()` nueva. Las specs inmobiliarias de estate (hab/baños/m²) van embebidas en la descripción — límite conocido de la aproximación por secciones, documentado para su futuro renderer custom.

**Fase 2 — Recetas (kit de pizza, Fase B del plan de temas).** `Template.stylePresets Json?` (misma migración), expuesto en el detalle vía entity→repo→use-case→DTO. Recetas autoradas: Vitrina Cálida, Noir Cálido, Noir Contraste, Menú Nocturno. Frontend: bloque "Recetas del tema" arriba de la edición libre en TokensEditor (que pasa a "Personalizar"), card "Original" derivada de defaultTokens (reset estético sin perder contenido), activa detectada por firma canónica de tokens. `handleApplyPreset` en theme-editor: tokens completos (deep-merge del hook = replace) + sectionOverrides que solo pisan visible/props declarados. Nota: el shape final usa `props` en los overrides, no `variant` — las variantes de layout son props normales desde el editor de secciones. Corrección de tipo en el camino: `TypographyTokens.scale` viajaba del seed sin estar tipado en el frontend.

**Fase 3 — Dispatcher + PersonaRenderer fiel (Fase E parcial).** `themes/registry.tsx`: ThemeRenderer despacha por `theme.template` sin props extra — swap en storefront-client, editor-canvas y theme-preview-modal (la página de producto NO se despacha: la provee la plataforma para todos los temas). PersonaRenderer recupera lo que la aproximación por secciones no podía: cover+avatar superpuesto, stats, TABS Servicios/Portfolio, grid 3-col tipo IG, FAB WhatsApp — adaptado al framework: tokens (no store.primaryColor), árbol de secciones respetado con selección del editor (seleccionar gallery/grid cambia a la tab correcta), secciones commodity DELEGADAS al SectionRenderer base (exportado para esto — patrón central del contrato), productHref a la página de producto, container queries. Bug propio en el swap: el replace de `<TemplateRenderer` alcanzó a `Omit<TemplateRendererProps` en storefront-client — corregido.
