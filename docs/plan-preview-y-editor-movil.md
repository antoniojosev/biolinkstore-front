...
6. **Comprobar en el editor de Diseño con una tienda sin productos**: confirmar visualmente que el canvas muestra tu `store` real (nombre/logo/redes) con productos/categorías DEMO de fallback (no vacío) — comportamiento ya implementado en `editor-canvas.tsx` (líneas ~26-56: `real.hasProducts` decide products/categories reales vs demo, `store` siempre es `real.store`), pedido por Antonio verificar con los ojos.
